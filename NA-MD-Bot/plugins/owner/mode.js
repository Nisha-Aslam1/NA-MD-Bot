...
// public = everyone, private = owner-only
// ============================================
...
export default {
  command: 'mode',
  alias: ['botmode'],
  description: 'Set bot mode for this number: public (everyone) or private (owner only)',
  category: 'owner',
  ownerOnly: true,
  async execute({ args, reply, sessionSettings }) {
    ...
    if (!mode || !['public', 'private'].includes(mode)) {
      return reply(
        ...
        `💡 Saved per connected number and restored after restart.`
      );
    }
    ...
    const desc  = mode === 'private'
      ? 'Only the owner can now use bot commands on *this number*.'
      : 'Everyone can now use bot commands on *this number*. This setting is saved and restored after restart.';
